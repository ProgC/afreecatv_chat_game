...
std::map<WORD, InputData*> mKeyMap;
...
void ProcessGameTime()
{
	...
	std::map<WORD, InputData*>::iterator it = mKeyMap.begin();
	while( it != mKeyMap.end() )
	{
		if ( (*it).second->mValid )
		{
			if ( curTime >= (*it).second->mTime )
			{
				// disable input and then delete it.
				INPUT input;
				
				memset(&input,0,sizeof(INPUT));
				input.type = INPUT_KEYBOARD;
				input.ki.wScan = (*it).second->mKey;	
...
void ProcessMoveKey(WORD key, DWORD time)
{
	std::map<WORD, InputData*>::iterator it = mKeyMap.find(key);
	...
	if ( it == mKeyMap.end() )
	{
		...
		memset(&input,0,sizeof(INPUT));
		input.type = INPUT_KEYBOARD;
		input.ki.wScan = key; // direct-input scancode for key '1'				
		input.ki.dwFlags = 0;
		SendInput(1,&input,sizeof(INPUT));
		
		mKeyMap.insert( std::pair<WORD, InputData*>( key, newInput ) );
	}
	else
	{
		INPUT input;
		...
		if ( (*it).second->mValid )
		{
			// 이미 키가 있다면 그 키를 해제하고 새로운 키를 등록한다.			
			memset(&input,0,sizeof(INPUT));
			input.type = INPUT_KEYBOARD;
			input.ki.wScan = key; // direct-input scancode for key '1'		
			input.ki.dwFlags = KEYEVENTF_KEYUP;		
...
char commandBuf[10];
void ProcessMessage(char* command, int size)
{
	if ( size > 0 )
	{
		if ( command[0] == '!' )
		{
			...
			{
				commandEnd++;
			} while(command[commandEnd] != ' ' && commandEnd <= size );
			
			strncpy( commandBuf, &command[1], commandEnd-1 );

			if ( strcmp( commandBuf, "move" ) == 0 )
			{
				printf( "move command is received\n" );
				...
				if ( size >= 9 )
				{
					...
					memset( commandBuf, 0, sizeof(commandBuf) );								
					strncpy( commandBuf, &command[8], digits );
					
					//if ( isdigit(atoi(commandBuf)) )
					{
						DWORD time = timeGetTime() + (atoi(commandBuf) * 300);
				
						// r, u, d, l
						if ( command[6] == 'r' )
						{
							ProcessMoveKey( DIKEYBOARD_RIGHT, time );
						}
						else if ( command[6] == 'u' )
						{
							ProcessMoveKey( DIKEYBOARD_UP, time );
						}
						else if ( command[6] == 'd' )
						{
							ProcessMoveKey( DIKEYBOARD_DOWN, time );
						}
						else if ( command[6] == 'l' )
						{
							ProcessMoveKey( DIKEYBOARD_LEFT, time );					
						}
					}
				}
			}
			else if ( strcmp( commandBuf, "enter" ) == 0 )
			{
				printf( "enter command is received\n" );
				
				ProcessMoveKey( DIKEYBOARD_RETURN, timeGetTime() + 1 );
			}
			else if ( strcmp( commandBuf, "action" ) == 0 )
			{
				printf( "action command is received\n" );
				
				if ( command[8] == 'a' )
				{
					// x key
					ProcessMoveKey( DIKEYBOARD_X, timeGetTime() + 1 );
				}
				else if ( command[8] == 'b' )
				{
					// z key
					ProcessMoveKey( DIKEYBOARD_Z, timeGetTime() + 1 );
				}
				else if ( command[8] == 'q' )
				{
					// left top, a key
					ProcessMoveKey( DIKEYBOARD_A, timeGetTime() + 1 );
				}
				else if ( command[8] == 'w' )
				{
					// right top, s key
					ProcessMoveKey( DIKEYBOARD_S, timeGetTime() + 1 );
				}
			}
		}		
	}	
...
// 보통은 win32api로 돌아가지만 visualboy advance는 입력으로 다이렉트 인풋을 사용한다.
// 다이렉트 인풋을 시뮬레이션 해줘야 한다.
int main()
{
	printf( "Afreeca social game server is started.\n" );
	printf( "by ProgC\n" );
	
	InitWinsock();

	// Receive message then process it.
